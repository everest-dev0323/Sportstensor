...
for league in leagues:
    ...
    avg_teams = pd.DataFrame({"Team": teams, "Elo_Score": [0] * len(teams)})
    ...
    def average_elo(group, team_name):
        ...
        return pd.Series({"Elo_Score": elo_score, "LastMatchDate": latest_match_date})

    grp_by_HT_and_season = E0_data.groupby(["HomeTeam", "Season"])
    grp_by_AT_and_season = E0_data.groupby(["AwayTeam", "Season"])

    avg_teams[["Team", "Elo_Score", "LastMatchDate"]] = (
        elo_df_reset.groupby("Team")
        .apply(lambda group: average_elo(group, group.name), include_groups=False)
        .reset_index()
    )
    ...
    def getTeamDay(row):
        ...
        if team in teams_by_season[cur_season]:
            ...
            if (team, cur_season) in grp_by_HT_and_season.groups:
                teamHomeResultsW_s = grp_by_HT_and_season.get_group((team, cur_season))[
                    "HW"
                ]
                teamHomeResults_s = grp_by_HT_and_season.get_group((team, cur_season))[
                    "HR"
                ]
                if league["can_draw"]:
            ...
            if (team, cur_season) in grp_by_AT_and_season.groups:
                teamAwayResultsW_s = grp_by_AT_and_season.get_group((team, cur_season))[
                    "AW"
                ]
                teamAwayResults_s = grp_by_AT_and_season.get_group((team, cur_season))[
                    "AR"
                ]
                if league["can_draw"]:
            ...
            teamResults_s = pd.concat(
                [teamHomeResults_s, teamAwayResults_s]
            ).sort_index()
            if league["can_draw"]:
            ...
            last_TR_isW_1 = (
                1
                if len(teamResults_s) >= 1 and np.asarray(teamResults_s)[-1] == "W"
                else 0
            )
            last_TR_isL_1 = (
                1
                if len(teamResults_s) >= 1 and np.asarray(teamResults_s)[-1] == "L"
                else 0
            )
            last_TR_isW_2 = (
                1
                if len(teamResults_s) >= 2 and np.asarray(teamResults_s)[-2] == "W"
                else 0
            )
            last_TR_isL_2 = (
                1
                if len(teamResults_s) >= 2 and np.asarray(teamResults_s)[-2] == "L"
                else 0
            )
            last_TR_isW_3 = (
                1
                if len(teamResults_s) >= 3 and np.asarray(teamResults_s)[-3] == "W"
                else 0
            )
            last_TR_isL_3 = (
                1
                if len(teamResults_s) >= 3 and np.asarray(teamResults_s)[-3] == "L"
                else 0
            )
            last_HTHR_isW_1 = (
                1
                if len(teamHomeResults_s) >= 1
                and np.asarray(teamHomeResults_s)[-1] == "W"
                else 0
            )
            last_HTHR_isL_1 = (
                1
                if len(teamHomeResults_s) >= 1
                and np.asarray(teamHomeResults_s)[-1] == "L"
                else 0
            )
            last_HTHR_isW_2 = (
                1
                if len(teamHomeResults_s) >= 2
                and np.asarray(teamHomeResults_s)[-2] == "W"
                else 0
            )
            last_HTHR_isL_2 = (
                1
                if len(teamHomeResults_s) >= 2
                and np.asarray(teamHomeResults_s)[-2] == "L"
                else 0
            )
            last_ATAR_isW_1 = (
                1
                if len(teamAwayResults_s) >= 1
                and np.asarray(teamAwayResults_s)[-1] == "W"
                else 0
            )
            last_ATAR_isL_1 = (
                1
                if len(teamAwayResults_s) >= 1
                and np.asarray(teamAwayResults_s)[-1] == "L"
                else 0
            )
            last_ATAR_isW_2 = (
                1
                if len(teamAwayResults_s) >= 2
                and np.asarray(teamAwayResults_s)[-2] == "W"
                else 0
            )
            last_ATAR_isL_2 = (
                1
                if len(teamAwayResults_s) >= 2
                and np.asarray(teamAwayResults_s)[-2] == "L"
                else 0
            )
            ...
            if len(teamResultsW_s) > 7:
                ...
                TW_rate_7 = (
                    np.asarray(teamResultsW_s).mean() if not teamResultsW_s.empty else 0
                )
            if league["can_draw"]:
                if len(teamResultsD_s) > 7:
                    ...
                    TD_rate_7 = (
                        np.asarray(teamResultsD_s).mean()
                        if not teamResultsD_s.empty
                        else 0
                    )
            ...
            if len(teamHomeResultsW_s) > 5:
                ...
                HTHW_rate_5 = (
                    np.asarray(teamHomeResultsW_s).mean()
                    if not teamHomeResultsW_s.empty
                    else 0
                )
            if len(teamAwayResultsW_s) > 5:
                ...
                ATAW_rate_5 = (
                    np.asarray(teamAwayResultsW_s).mean()
                    if not teamAwayResultsW_s.empty
                    else 0
                )
        if league["can_draw"]:
            return (
    ...
    avg_teams.to_csv(
        f"./match_infos/{league['folder_name']}/avg_teams_info.csv",
        index=False,
        encoding="utf-8",
    )
