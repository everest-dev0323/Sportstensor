...
for league in leagues:
    ...
    rf_params = {
        ...
        'feature_selection__max_features': randint(8, len(features1) if league['can_draw'] else len(features2))
    }
    ...
    x_train, x_test, y_train, y_test = train_test_split(X_imputed, Y, shuffle=False, test_size=0.3)

    # Perform random search for each model (except SVC)
    best_models = {}
    for name, (pipeline, params) in models.items():
        print(f"### Tuning {name}...")
        random_search = RandomizedSearchCV(pipeline, params, n_iter=80, cv=5, n_jobs=-1, verbose=1, random_state=42)
    ...
    confidence_percentage = y_pred_proba.max(axis=1) * 100
    cnt = 0
    total_cnt = 0
    for pred, test, confidence in zip(y_pred, y_test, confidence_percentage):
        if confidence > 50:
            total_cnt += 1
            if pred == test:
                cnt += 1

    print(f"Prediction Count: {len(x_test)} Total count: {cnt}, Pro: {cnt/total_cnt}")

    # Create a figure and axis
    # plt.figure(figsize=(12, 6))

    # plt.plot(y_pred, label='Predicted (Testing)', color='blue', linestyle='-', marker='o')
    # plt.plot(np.array(y_test), label='Actual (Testing)', color='red', linestyle='-', marker='x')
