...
UPCOMMING_MATCHES_PATH = './data/next_matches.csv'
...
class Predictor:
    ...
    def get_match_features(self, home_team, away_team, match_date, avg_team_df, can_draw):
        ...
        if can_draw:
            df = df.merge(
                avg_team_df.rename(columns={'Team': 'HomeTeam', 'Elo_Score': 'HTeamEloScore', 
                                            'TW_rate': 'HTW_rate','TD_rate': 'HTD_rate',
                                            'TW_rate_7': '7_HTW_rate','TD_rate_7': '7_HTD_rate','TL_rate_7': '7_HTL_rate',
                                            'TW_rate_12': '12_HTW_rate','TD_rate_12': '12_HTD_rate','TL_rate_12': '12_HTL_rate',
                                            'HTHW_rate_5': '5_HTHW_rate', 'LastMatchDate': 'HTLastMatchDate'}),
                how='left',
                left_on=['HomeTeam'],
                right_on=['HomeTeam']
            )
            ...
            df = df.merge(
                avg_team_df.rename(columns={'Team': 'AwayTeam', 'Elo_Score': 'ATeamEloScore', 
                                            'TW_rate': 'ATW_rate','TD_rate': 'ATD_rate',
                                            'TW_rate_7': '7_ATW_rate','TD_rate_7': '7_ATD_rate','TL_rate_7': '7_ATL_rate',
                                            'TW_rate_12': '12_ATW_rate','TD_rate_12': '12_ATD_rate','TL_rate_12': '12_ATL_rate',
                                            'ATAW_rate_5': '5_ATAW_rate', 'LastMatchDate': 'ATLastMatchDate'}),
                how='left',
                left_on=['AwayTeam'],
                right_on=['AwayTeam']
            )
            ...
            df = df.merge(
                avg_team_df.rename(columns={'Team': 'HomeTeam', 'Elo_Score': 'HTeamEloScore', 
                                            'TW_rate': 'HTW_rate',
                                            'TW_rate_7': '7_HTW_rate','TL_rate_7': '7_HTL_rate',
                                            'TW_rate_12': '12_HTW_rate','TL_rate_12': '12_HTL_rate',
                                            'HTHW_rate_5': '5_HTHW_rate', 'LastMatchDate': 'HTLastMatchDate'}),
                how='left',
                left_on=['HomeTeam'],
                right_on=['HomeTeam']
            ...
            df = df.merge(
                avg_team_df.rename(columns={'Team': 'AwayTeam', 'Elo_Score': 'ATeamEloScore', 
                                            'TW_rate': 'ATW_rate',
                                            'TW_rate_7': '7_ATW_rate','TL_rate_7': '7_ATL_rate',
                                            'TW_rate_12': '12_ATW_rate','TL_rate_12': '12_ATL_rate',
                                            'ATAW_rate_5': '5_ATAW_rate', 'LastMatchDate': 'ATLastMatchDate'}),
                how='left',
                left_on=['AwayTeam'],
                right_on=['AwayTeam']
    ...
    def get_match_odds(self, home_team, away_team, league, can_draw):
        ...
        cur_match = df[(df['League'] == league) & (df['HomeTeam'] == home_team) & (df['AwayTeam'] == away_team)]
        if cur_match.empty:
            return None
        if can_draw:
    ...
    def predict_winner(self, prediction):
        ...
        competition = next((league for league in self.leagues if league["league_name"] == competition), None)
        if competition is None:
            if sport == 1:
                ...
                if (home_match is None) or (away_match is None):
                    competition = next((league for league in self.leagues if league["league_name"] == 'English Premier League'), None)
                else:
                    competition = next((league for league in self.leagues if league["league_name"] == 'American Major League Soccer'), None)
            else:
                competition = next((league for league in self.leagues if league["sport_type"] == sport), None)
        ...
        fbmodel = tf.keras.models.load_model(f"{model_path}/model.keras", custom_objects={'TimeStepSlice': TimeStepSlice})
        le = joblib.load(f"{model_path}/label_encoder.joblib")
        encoded_teams = set(le.classes_)
        home_match = self.get_best_match(home_team, encoded_teams)
        away_match = self.get_best_match(away_team, encoded_teams)

        if (home_team in mismatch_teams.keys()) and (competition['league_name'] == 'NFL'):
            home_match = mismatch_teams[home_team]

        if (away_team in mismatch_teams.keys()) and (competition['league_name'] == 'NFL'):
        ...
        cur_match = self.get_match_features(home_match, away_match, match_date, avg_teams, can_draw)
        ...
        if result == 0:
            return home_team, conf_scores[0], np.array(odds), ProbabilityChoice.HOMETEAM
        elif result == 1:
            return away_team, conf_scores[0], np.array(odds), ProbabilityChoice.AWAYTEAM
        else:
            return 'DRAW', conf_scores[0], np.array(odds), ProbabilityChoice.DRAW
