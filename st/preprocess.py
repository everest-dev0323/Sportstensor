...
for league in leagues:
    ...
    # create teams list
    teams = pd.concat(
        [E0_data["HomeTeam"], E0_data["AwayTeam"]], ignore_index=True
    ).unique()
    # create seasons list
    seasons = np.sort(E0_data["Season"].unique())
    # create dictionary containing teams list by season
    teams_by_season = {
        season: E0_data[E0_data["Season"] == season]["HomeTeam"].unique()
    ...
    def getTeamDday(row, position):
        ...
        if x in E0_HT_grpby.groups:
            ...
            df1 = pd.DataFrame(
                columns=["Date"]
            )  # Empty DataFrame if team has no HomeTeam history
        ...
        if x in E0_AT_grpby.groups:
            ...
            df2 = pd.DataFrame(
                columns=["Date"]
            )  # Empty DataFrame if team has no AwayTeam history
        day = len(df1) + len(df2)
        return day
    ...
    for season in seasons:
        for team in teams_by_season[season]:
            sH = E0_data[(E0_data["HomeTeam"] == team) & (E0_data["Season"] == season)][
                "ones"
            ]
            E0_data.loc[sH.index, "HomeTeamHomeDay"] = sH.cumsum()

            sA = E0_data[(E0_data["AwayTeam"] == team) & (E0_data["Season"] == season)][
                "ones"
            ]
    ...
    for team in teams:
        ...
        if team in grp_by_HT.groups:
            ...
            teamHomeResults_s = pd.Series(
                dtype="int"
            )  # Empty series if no HomeTeam results
        if team in grp_by_AT.groups:
            teamAwayResults_s = grp_by_AT.get_group(team)["AR"]
        else:
            teamAwayResults_s = pd.Series(
                dtype="int"
            )  # Empty series if no AwayTeam results
        ...
        E0_data.loc[teamHomeResults_s.index, "1_last_HTR"] = E0_data.loc[
            teamHomeResults_s.index, :
        ].index.map(lambda x: lag1TeamResults_d[x])
        E0_data.loc[teamHomeResults_s.index, "2_last_HTR"] = E0_data.loc[
            teamHomeResults_s.index, :
        ].index.map(lambda x: lag2TeamResults_d[x])
        E0_data.loc[teamHomeResults_s.index, "3_last_HTR"] = E0_data.loc[
            teamHomeResults_s.index, :
        ].index.map(lambda x: lag3TeamResults_d[x])
        E0_data.loc[teamAwayResults_s.index, "1_last_ATR"] = E0_data.loc[
            teamAwayResults_s.index, :
        ].index.map(lambda x: lag1TeamResults_d[x])
        E0_data.loc[teamAwayResults_s.index, "2_last_ATR"] = E0_data.loc[
            teamAwayResults_s.index, :
        ].index.map(lambda x: lag2TeamResults_d[x])
        E0_data.loc[teamAwayResults_s.index, "3_last_ATR"] = E0_data.loc[
            teamAwayResults_s.index, :
        ].index.map(lambda x: lag3TeamResults_d[x])
        ...
        E0_data.loc[teamHomeResults_s.index, "1_last_HTHR"] = E0_data.loc[
            teamHomeResults_s.index, :
        ].index.map(lambda x: lag1TeamHomeResults_d[x])
        E0_data.loc[teamHomeResults_s.index, "2_last_HTHR"] = E0_data.loc[
            teamHomeResults_s.index, :
        ].index.map(lambda x: lag2TeamHomeResults_d[x])
        E0_data.loc[teamAwayResults_s.index, "1_last_ATAR"] = E0_data.loc[
            teamAwayResults_s.index, :
        ].index.map(lambda x: lag1TeamAwayResults_d[x])
        E0_data.loc[teamAwayResults_s.index, "2_last_ATAR"] = E0_data.loc[
            teamAwayResults_s.index, :
        ].index.map(lambda x: lag2TeamAwayResults_d[x])
        ...
        for season in seasons:
            ...
            if team in teams_by_season[season]:
                ...
                if (team, season) in grp_by_HT_and_season.groups:
                    teamHomeResultsW_s = grp_by_HT_and_season.get_group((team, season))[
                        "HW"
                    ]
                    if league["can_draw"]:
                        teamHomeResultsD_s = grp_by_HT_and_season.get_group(
                            (team, season)
                        )["D"]
                else:
                ...
                if (team, season) in grp_by_AT_and_season.groups:
                    teamAwayResultsW_s = grp_by_AT_and_season.get_group((team, season))[
                        "AW"
                    ]
                    if league["can_draw"]:
                        teamAwayResultsD_s = grp_by_AT_and_season.get_group(
                            (team, season)
                        )["D"]
                else:
                ...
                teamResultsW_s = pd.concat(
                    [teamHomeResultsW_s, teamAwayResultsW_s]
                ).sort_index()
                if league["can_draw"]:
                    teamResultsD_s = pd.concat(
                        [teamHomeResultsD_s, teamAwayResultsD_s]
                    ).sort_index()
                ...
                E0_data.loc[teamHomeResultsW_s.index, "HTW_rate"] = E0_data.loc[
                    teamHomeResultsW_s.index, :
                ].index.map(lambda x: teamResultsWCumul_d[x])
                E0_data.loc[teamAwayResultsW_s.index, "ATW_rate"] = E0_data.loc[
                    teamAwayResultsW_s.index, :
                ].index.map(lambda x: teamResultsWCumul_d[x])
                if league["can_draw"]:
                    E0_data.loc[teamHomeResultsW_s.index, "HTD_rate"] = E0_data.loc[
                        teamHomeResultsW_s.index, :
                    ].index.map(lambda x: teamResultsDCumul_d[x])
                    E0_data.loc[teamAwayResultsW_s.index, "ATD_rate"] = E0_data.loc[
                        teamAwayResultsW_s.index, :
                    ].index.map(lambda x: teamResultsDCumul_d[x])

                E0_data.loc[teamHomeResultsW_s.index, "7_HTW_rate"] = E0_data.loc[
                    teamHomeResultsW_s.index, :
                ].index.map(lambda x: win7TeamResultsW_d[x])
                E0_data.loc[teamHomeResultsW_s.index, "12_HTW_rate"] = E0_data.loc[
                    teamHomeResultsW_s.index, :
                ].index.map(lambda x: win12TeamResultsW_d[x])
                E0_data.loc[teamAwayResultsW_s.index, "7_ATW_rate"] = E0_data.loc[
                    teamAwayResultsW_s.index, :
                ].index.map(lambda x: win7TeamResultsW_d[x])
                E0_data.loc[teamAwayResultsW_s.index, "12_ATW_rate"] = E0_data.loc[
                    teamAwayResultsW_s.index, :
                ].index.map(lambda x: win12TeamResultsW_d[x])
                if league["can_draw"]:
                    E0_data.loc[teamHomeResultsD_s.index, "7_HTD_rate"] = E0_data.loc[
                        teamHomeResultsD_s.index, :
                    ].index.map(lambda x: win7TeamResultsD_d[x])
                    E0_data.loc[teamAwayResultsD_s.index, "7_ATD_rate"] = E0_data.loc[
                        teamAwayResultsD_s.index, :
                    ].index.map(lambda x: win7TeamResultsD_d[x])
                    E0_data.loc[teamHomeResultsD_s.index, "12_HTD_rate"] = E0_data.loc[
                        teamHomeResultsD_s.index, :
                    ].index.map(lambda x: win12TeamResultsD_d[x])
                    E0_data.loc[teamAwayResultsD_s.index, "12_ATD_rate"] = E0_data.loc[
                        teamAwayResultsD_s.index, :
                    ].index.map(lambda x: win12TeamResultsD_d[x])

                # (ii) compute 5_HTHW_rate and 5_ATAW_rate
                win5TeamResultsHomeW_d = (
                    teamHomeResultsW_s.shift(1)
                    .rolling(window=5, min_periods=3)
                    .mean()
                    .to_dict()
                )
                win5TeamResultsAwayW_d = (
                    teamAwayResultsW_s.shift(1)
                    .rolling(window=5, min_periods=3)
                    .mean()
                    .to_dict()
                )
                E0_data.loc[teamHomeResultsW_s.index, "5_HTHW_rate"] = E0_data.loc[
                    teamHomeResultsW_s.index, :
                ].index.map(lambda x: win5TeamResultsHomeW_d[x])
                E0_data.loc[teamAwayResultsW_s.index, "5_ATAW_rate"] = E0_data.loc[
                    teamAwayResultsW_s.index, :
                ].index.map(lambda x: win5TeamResultsAwayW_d[x])
                ...
                if league["can_draw"]:
                    teamHomeResultsCumulD_d = (
                        teamHomeResultsD_s.shift(1).cumsum().to_dict()
                    )
                    teamAwayResultsCumulD_d = (
                        teamAwayResultsD_s.shift(1).cumsum().to_dict()
                    )
                E0_data.loc[teamHomeResultsW_s.index, "HTHW_rate"] = E0_data.loc[
                    teamHomeResultsW_s.index, :
                ].index.map(lambda x: teamHomeResultsCumulW_d[x])
                E0_data.loc[teamAwayResultsW_s.index, "ATAW_rate"] = E0_data.loc[
                    teamAwayResultsW_s.index, :
                ].index.map(lambda x: teamAwayResultsCumulW_d[x])
                if league["can_draw"]:
                    E0_data.loc[teamHomeResultsW_s.index, "HTHD_rate"] = E0_data.loc[
                        teamHomeResultsW_s.index, :
                    ].index.map(lambda x: teamHomeResultsCumulD_d[x])
                    E0_data.loc[teamAwayResultsW_s.index, "ATAD_rate"] = E0_data.loc[
                        teamAwayResultsW_s.index, :
                    ].index.map(lambda x: teamAwayResultsCumulD_d[x])

    # compute missing features k_XTL_rate thanks to the k_XTW_rate and k_XTD_rate features
    if league["can_draw"]:
        E0_data.loc[:, "7_HTL_rate"] = 1 - (
            E0_data["7_HTW_rate"] + E0_data["7_HTD_rate"]
        )
        E0_data.loc[:, "12_HTL_rate"] = 1 - (
            E0_data["7_HTW_rate"] + E0_data["7_HTD_rate"]
        )
        E0_data.loc[:, "7_ATL_rate"] = 1 - (
            E0_data["7_ATW_rate"] + E0_data["7_ATD_rate"]
        )
        E0_data.loc[:, "12_ATL_rate"] = 1 - (
            E0_data["7_ATW_rate"] + E0_data["7_ATD_rate"]
        )
    else:
    ...
    E0_data.loc[:, "HTW_rate"] = np.where(
        E0_data["HomeTeamDay"] != 0, E0_data["HTW_rate"] / E0_data["HomeTeamDay"], 0
    )
    E0_data.loc[:, "ATW_rate"] = np.where(
        E0_data["AwayTeamDay"] != 0, E0_data["ATW_rate"] / E0_data["AwayTeamDay"], 0
    )
    if league["can_draw"]:
        E0_data.loc[:, "HTD_rate"] = np.where(
            E0_data["HomeTeamDay"] != 0, E0_data["HTD_rate"] / E0_data["HomeTeamDay"], 0
        )
        E0_data.loc[:, "ATD_rate"] = np.where(
            E0_data["AwayTeamDay"] != 0, E0_data["ATD_rate"] / E0_data["AwayTeamDay"], 0
        )
    ...
    E0_data.loc[:, "HTHW_rate"] = np.where(
        E0_data["HomeTeamHomeDay"] != 0,
        E0_data["HTHW_rate"] / E0_data["HomeTeamHomeDay"],
        0,
    )
    E0_data.loc[:, "ATAW_rate"] = np.where(
        E0_data["AwayTeamAwayDay"] != 0,
        E0_data["ATAW_rate"] / E0_data["AwayTeamAwayDay"],
        0,
    )
    if league["can_draw"]:
        E0_data.loc[:, "HTHD_rate"] = np.where(
            E0_data["HomeTeamHomeDay"] != 0,
            E0_data["HTHD_rate"] / E0_data["HomeTeamHomeDay"],
            0,
        )
        E0_data.loc[:, "ATAD_rate"] = np.where(
            E0_data["AwayTeamAwayDay"] != 0,
            E0_data["ATAD_rate"] / E0_data["AwayTeamAwayDay"],
            0,
        )
    ...
    for season in E0_data["Season"].unique():
        ...
        if len(season_elos) >= 17:
            last_teams_ELO_av = np.mean(
                np.sort(season_elos)[:-17]
            )  # Exclude the bottom 17 Elo scores
        else:
            last_teams_ELO_av = np.mean(
                np.sort(season_elos)
            )  # Exclude the bottom 17 Elo scores
    ...
    for team in teams:
        ...
        E0_data.loc[E0_data["HomeTeam"] == team, "HTLastMatchDate"] = E0_data.loc[
            E0_data["HomeTeam"] == team
        ].index.map(lambda x: lastMatchDates_s[x])
        E0_data.loc[E0_data["AwayTeam"] == team, "ATLastMatchDate"] = E0_data.loc[
            E0_data["AwayTeam"] == team
        ].index.map(lambda x: lastMatchDates_s[x])
    ...
    E0_data.loc[:, "DaysSinceLastMatchRate"] = np.where(
        E0_data["ATdaysSinceLastMatch"] != 0,
        E0_data["HTdaysSinceLastMatch"] / E0_data["ATdaysSinceLastMatch"],
        0,
    )

    E0_data["1_last_HTR_isW"] = E0_data["1_last_HTR"].map(
        lambda x: 1 if x == "W" else 0
    )
    E0_data["1_last_HTR_isL"] = E0_data["1_last_HTR"].map(
        lambda x: 1 if x == "L" else 0
    )
    E0_data["2_last_HTR_isW"] = E0_data["2_last_HTR"].map(
        lambda x: 1 if x == "W" else 0
    )
    E0_data["2_last_HTR_isL"] = E0_data["2_last_HTR"].map(
        lambda x: 1 if x == "L" else 0
    )
    E0_data["3_last_HTR_isW"] = E0_data["3_last_HTR"].map(
        lambda x: 1 if x == "W" else 0
    )
    E0_data["3_last_HTR_isL"] = E0_data["3_last_HTR"].map(
        lambda x: 1 if x == "L" else 0
    )

    E0_data["1_last_ATR_isW"] = E0_data["1_last_ATR"].map(
        lambda x: 1 if x == "W" else 0
    )
    E0_data["1_last_ATR_isL"] = E0_data["1_last_ATR"].map(
        lambda x: 1 if x == "L" else 0
    )
    E0_data["2_last_ATR_isW"] = E0_data["2_last_ATR"].map(
        lambda x: 1 if x == "W" else 0
    )
    E0_data["2_last_ATR_isL"] = E0_data["2_last_ATR"].map(
        lambda x: 1 if x == "L" else 0
    )
    E0_data["3_last_ATR_isW"] = E0_data["3_last_ATR"].map(
        lambda x: 1 if x == "W" else 0
    )
    E0_data["3_last_ATR_isL"] = E0_data["3_last_ATR"].map(
        lambda x: 1 if x == "L" else 0
    )

    E0_data["1_last_HTHR_isW"] = E0_data["1_last_HTHR"].map(
        lambda x: 1 if x == "W" else 0
    )
    E0_data["1_last_HTHR_isL"] = E0_data["1_last_HTHR"].map(
        lambda x: 1 if x == "L" else 0
    )
    E0_data["2_last_HTHR_isW"] = E0_data["2_last_HTHR"].map(
        lambda x: 1 if x == "W" else 0
    )
    E0_data["2_last_HTHR_isL"] = E0_data["2_last_HTHR"].map(
        lambda x: 1 if x == "L" else 0
    )

    E0_data["1_last_ATAR_isW"] = E0_data["1_last_ATAR"].map(
        lambda x: 1 if x == "W" else 0
    )
    E0_data["1_last_ATAR_isL"] = E0_data["1_last_ATAR"].map(
        lambda x: 1 if x == "L" else 0
    )
    E0_data["2_last_ATAR_isW"] = E0_data["2_last_ATAR"].map(
        lambda x: 1 if x == "W" else 0
    )
    E0_data["2_last_ATAR_isL"] = E0_data["2_last_ATAR"].map(
        lambda x: 1 if x == "L" else 0
    )
    ...
    for season in seasons:
        ...
        HW_rate = float(len(season_data[season_data["FTR"] == "H"].index)) / float(
            total_matches_nb
        )
        AW_rate = float(len(season_data[season_data["FTR"] == "A"].index)) / float(
            total_matches_nb
        )
        HW_rates.append(HW_rate)
        AW_rates.append(AW_rate)
        if league["can_draw"]:
            D_rate = float(len(season_data[season_data["FTR"] == "D"].index)) / float(
                total_matches_nb
            )
